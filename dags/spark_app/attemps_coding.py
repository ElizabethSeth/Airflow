...
def preprocess_data(spark, path_data):
    raw_df = spark.read.text(path_data)
    ...
    process_df = data_df.withColumn("split_values", F.split(F.col("value"), ","))
    process_df = process_df.select(
        [F.col("split_values").getItem(i).alias(column_names[i]) for i in range(len(column_names))]
    )
...
def main():
    ...
    process_df, column_names = preprocess_data(spark, f"{path_data}/HI-Large_Patterns.txt")
